...
Subtask 规划策略由 `autoSubtaskMode` 插件设置控制：`off` 禁用新 root task 的自动计划，`suggest` 只记录可审阅建议，`auto` 会从建议中创建已关联 child tasks。项目级覆盖可在 `.omp/plugin-overrides.json` 或 `.pi/plugin-overrides.json` 中设置 `settings["omp-project-flow"].autoSubtaskMode`。`/task:subtasks --mode off|suggest|auto` 可对单个任务用显式策略重新生成计划。`/task:subtasks --template acceptance|workflow|roles|verification --depth N` 会选择 deterministic split templates，并记录顺序、依赖、嵌套 parent plan-item links 和 tree rollups。
...
Research artifacts 位于每个 task 的 `research/` 目录下。普通 notes 仍在 `research/notes.md`，已审阅证据可作为 source packs 写入 `research/source-packs.json`，包含 kind、source path/URL、claim、excerpt、confidence 和 open risks。使用 `/research:add-source --source <path-or-url> --claim <claim> [--excerpt <text>] [--confidence low|medium|high]` 添加已审阅证据，使用 `/research:summary` 查看摘要。Upstream/parity task 如果没有 reviewed source pack，会得到 readiness warning；这是审阅提示，不代表 Project Flow 自动完成了 research。
...
- 新增 reviewed research source packs，记录 source kind、claim、excerpt、confidence 和 open risks。
- Research source 摘要现在会进入 task info、handoff、snapshot、hidden context，并为 upstream/parity work 提供 readiness warning。

### 0.23.0

- 新增 verification policy matrix，用 touched files、建议 checks 和已记录 pass evidence 做覆盖判断。
- Readiness 和 resume packs 现在会把 verification coverage gaps 展示为可审阅 next actions。

### 0.22.0

- 新增更丰富的 subtask planning templates：acceptance、workflow、roles 和 verification。
- Subtask plan 现在保存 item order、depth、dependencies、parent item links、template 和 max depth。
- Subtask tree 现在展示按 status、phase、depth、leaf、truncation 和 blocked subtasks 汇总的 rollups。
